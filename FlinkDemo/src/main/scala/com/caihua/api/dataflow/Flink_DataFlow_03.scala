...
object Flink_DataFlow_03 {
  def main(args: Array[String]): Unit = {
    ...
    //　TODO：startNewChain()：从当前算子开始产生一个新的任务链
    val wordToOneDS: DataStream[String] = linesDS.flatMap(_.split(","))//.startNewChain()

    //5.变换数据类型
    // TODO：disableChaining():表示当前算子不使用链条，独立的Task
    val wordToTupleDS: DataStream[(String, Int)] = wordToOneDS.map((_,1)).disableChaining()
